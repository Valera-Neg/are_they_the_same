...
have the "same" elements, with the same multiplicities
 (the multiplicity of a member is the number of times 
  it appears). "Same" means, here, that the elements 
...
const arr1 = array1.sort((a,b) => a-b).map(e => Math.pow(e, 2));
const arr2 = array2.sort((a,b) => a-b)
  if (arr1.length != arr2.length) {
    return false;
  }
  for (let i=0; i<arr2.length; i++) {     
    if (arr2[i] != arr1[i] || arr2[i] === null || arr1[i] === null) {
     return false;
    }   
  }
 return true;
